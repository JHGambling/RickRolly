...
function drawBettingTable() {
    ...
    tableCtx.fillRect(
        zeroCell.x + tableMargin,
        zeroCell.y + tableMargin,
        zeroCell.w,
        zeroCell.h,
    );
    ...
    tableCtx.fillText(
        "0",
        zeroCell.x + tableMargin + cellW / 2,
        tableMargin + cellH * 1.5,
    );
    tableCtx.restore();
    ...
    for (let col = 0; col < cols; col++) {
        for (let row = 0; row < rows; row++) {
            ...
            tableCtx.fillStyle =
                colors[numbers.indexOf(n)] === "red"
                    ? "#e63946"
                    : colors[numbers.indexOf(n)] === "black"
                      ? "#222"
                      : "#2e8b57";
...
async function placeOrRemoveChip(x, y, numberOrColor) {
    ...
    if (betIdx !== -1) {
        ...
        renderChips();
        betInfo.textContent =
            typeof numberOrColor === "number"
                ? `Bet removed from number ${numberOrColor}`
                : `Bet removed from color ${numberOrColor}`;
        return;
    }
    ...
    betInfo.textContent =
        typeof numberOrColor === "number"
            ? `Bet placed on number ${numberOrColor} ($${betAmount})`
            : `Bet placed on color ${numberOrColor} ($${betAmount})`;
}
...
function getNumberFromTableClick(x, y) {
    ...
    if (
        x >= tableMargin + cellW &&
        x < tableMargin + cellW * 13 &&
        y >= tableMargin &&
        y < tableMargin + cellH * 3
    ) {
        const col = Math.floor((x - (tableMargin + cellW)) / cellW);
        const row = Math.floor((y - tableMargin) / cellH);
...
function spinWheel() {
    ...
    if (!currentBets.length) {
        betInfo.textContent =
            "Please place at least one bet by clicking the table!";
        return;
    }
    spinning = true;
    showRouletteWheel();
...
async function showResultMulti() {
    ...
    currentBets.forEach((bet) => {
        ...
        if (win) {
            totalWin += payout;
            winDetails.push(
                `${bet.type === "number" ? bet.value : bet.value.charAt(0).toUpperCase() + bet.value.slice(1)} ($${bet.amount} → $${payout})`,
            );
        } else {
            lossDetails.push(
                `${bet.type === "number" ? bet.value : bet.value.charAt(0).toUpperCase() + bet.value.slice(1)} ($${bet.amount})`,
            );
        }
    });
    ...
    if (animType === "loss" && playerBudget === 0) {
        if (bailoutStage < bailouts.length) {
            ...
            if (!bailoutBtn) {
                ...
                bailoutBtn.style.background =
                    "linear-gradient(90deg,#ffd700,#e63946,#ffd700)";
                ...
                bailoutBtn.addEventListener("click", () => {
                    ...
                    betInfo.textContent = bailouts[bailoutStage].label.replace(
                        / for .*/,
                        " acquired! Good luck!",
                    );
                    bailoutStage++;
...
function showWinAnim(type, amount, onDone) {
    ...
    if (type === "loss") {
        content.innerHTML =
            '<span class="win-anim-sad">😢<br>Better luck next time!</span>';
        overlay.style.background = "rgba(33,150,243,0.18)";
    } else if (type === "color") {
        content.innerHTML =
            '<span class="win-anim-happy">🎉<br>Nice! You won by color!<br>+$' +
            amount +
            "</span>";
        ...
        content.innerHTML =
            '<span class="win-anim-jackpot">🎊<br>Jackpot! Number win!<br>+$' +
            amount +
            "</span>";
        ...
        content.innerHTML =
            '<span class="win-anim-crown">👑</span><span class="win-anim-jackpot">JACKPOT!<br>Zero Win!<br>+$' +
            amount +
            "</span>";
...
function confettiEffect(count, ...colors) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        conf.style.transition =
            "transform 1.7s cubic-bezier(.23,1.02,.64,1), opacity 1.7s";
        conf.style.color =
            colors[Math.floor(Math.random() * colors.length)] || "#ffd700";
